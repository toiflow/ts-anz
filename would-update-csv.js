#!/usr/bin/env node
// would-update-csv.js â€” append daily asset analysis row to would/LOG-METRIC-V1.csv
...
async function githubPut(path, sha, content, message) {
  const body = { message, content: Buffer.from(content).toString('base64'), committer: { name: 'gs-anz', email: 'jayreck996@gmail.com' } };
  if (sha) body.sha = sha;
  const res = await fetch(
    `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${path}`,
    {
