#!/usr/bin/env node
// would-read-md.js — fetch NZ interest rate news from RSS, print news context to stdout

const RSS_URLS = [
  'https://news.google.com/rss/search?q=New+Zealand+interest+rate+OCR+RBNZ&hl=en-NZ&gl=NZ&ceid=NZ:en',
  'https://news.google.com/rss/search?q=New+Zealand+monetary+policy+inflation&hl=en-NZ&gl=NZ&ceid=NZ:en'
];

const KEYWORDS = ['interest rate', 'ocr', 'official cash rate', 'monetary policy', 'inflation', 'reserve bank', 'rbnz'];
...
async function main() {
  ...
  for (const url of RSS_URLS) {
    try {
      const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; gs-anz/1.0)' } });
